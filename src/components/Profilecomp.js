...
const Profilecomp = ({setprofilePics}) => {
  ...
  let surnName = profileData.surname || profileData.lastname;
  ...
  const [userProfileData, setuserProfileData] = useState('text absolute w-5/6 md:top-52 top-48 mt-10 mx-10 pr-5')
  ...
  const handleCancelPic = () => {
    ...
    setpicsClass("hidden");
    setoptionBar('flex');
    setuserProfileData('text absolute w-5/6 top-52 mt-10 mx-10 pr-5');
  }
...
window.alert("Sorry Due to some internal error you are unable to log out")
  ...
  return (
    <div>
       ...
       <div className=' bg-gray-100 lg:ml-96 w-full lg:4/5  md:w-full  md:h-screen dark:bg-slate-900  dark:text-white text-md'>
                 <div className="text w-full md:mx-60 lg:mx-96 md:w-4/5  mx-auto 
                lg:w-2/5 border h-screen bg-gray-50 dark:bg-slate-800 dark:text-white  border-t-0 fixed ">
                      <div className="text md:w-4/5 lg:w-2/5 w-full h-40 bg-slate-400 dark:bg-slate-900 px-10 fixed">
                        <div className={optionBar}>
                          <Link className='text md:hidden'>
                                <FaBars className='absolute right-10 top-16 text-xl text-white' onClick={handleSideNav}/>
                          </Link>
                        </div>
                      </div>
                      {/* User profile picture */}
                      <div className="text w-36 h-36 bg-slate-200 border rounded-full z-10 fixed translate-y-20 translate-x-5 p-1 dark:bg-slate-800" onDoubleClick={handleWideView}  onTouchStart={handleWideView}>
                        {!downloadUrl? 
                        <img src={img} alt="" className='h-full w-full rounded-full' />:
                        <img src={downloadUrl} alt=" " className='h-full w-full rounded-full' />
                      }
                      </div>
                      <FaCamera className=' z-20  text-black absolute dark:text-white  top-40 left-52 -translate-x-14 cursor-pointer text-xl' title='Upload Profile' onClick={handleProfilePics}></FaCamera>
                          ...
                          <div className="w-full">
                                    <ul className="text mx-auto  border-b border-b-gray-500  pb-10 w-4/5 lg:mx-auto">
                                        <NavLink to='/dashboard' exact activeclassName={active} className={'flex hover:font-bold hover:transition hover:underline underline-offset-2 ease-in-out md:-translate-x-16 hover:scale-110 '}>
                                                <FaTachometerAlt className="text-sm mr-3  my-5"/>
                                    ...
                                    <ul className="text mt-5  w-4/5 mx-auto">
                                        ...
                                        <NavLink exact activeclassName={active} className={'flex hover:font-bold hover:transition hover:underline underline-offset-2 ease-in-out md:-translate-x-16 hover:scale-110 '}>
                                                <FaSignOutAlt className="text-sm mr-3 my-64  text-red-700"/>
                                                <li className="border-gray-400 text-sm my-4 text-red-700" onClick={handleLogOut}>LOGOUT</li>
                                        </NavLink>
                                    </ul>
                              </div>
                          </div>
